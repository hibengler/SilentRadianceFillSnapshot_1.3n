...
SDL_Init(SDL_INIT_AUDIO);
    ...
    if ( SDL_OpenAudio(&wanted, &obtained) < 0 ) {
        logit( "Couldn't open audio: %s\n", SDL_GetError());
        return(-1);
    }
    if (obtained.freq != wanted.freq) {
      logit("freq is %d",obtained.freq);
      }
    if (obtained.format != wanted.format) {
      logit("format is %d",obtained.format);
      }
    if (obtained.samples!= wanted.samples) {
      logit("samples is %d",obtained.samples);
      }
...
logit("setting up opus\n");
  
logit("setting up zero buffer\n");
sound_state=0;
{
  int i;
  for (i=0;i<480;i++) {

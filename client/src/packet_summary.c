...
struct bpg_image *bpg_images[65536];

unsigned short bpg_image_queue[BPG_IMAGE_QUEUE_SIZE];
int bpg_image_queue_head=0;
int bpg_image_queue_tail=0;
#endif
...
/* this command lods a bpg image into the list of images - up to 65536 
...
*/
void I_set_bpg_image_command(struct packet *packet,unsigned char *command,int length)
{
...
//       logit("duplicate image- keep building\n");
  ...
  bpg_images[image_id]=NULL;
  free(image);  
  }

image= malloc(sizeof(struct bpg_image));
...
void i_show_bpg_image_command(struct packet *packet,unsigned char *command,int length)
{
...
//     logit("no image id %d\n",image_id);
   ...
   for (int i=bpg_image_queue_head;i!=bpg_image_queue_tail; i = (i+1)%BPG_IMAGE_QUEUE_SIZE) {
     if (image_id == bpg_image_queue[i]) {
       logit("there");
       return; // already there
       }
     }
//   logit("we got %d\n",image_id);
   bpg_image_queue[bpg_image_queue_tail]=image_id;
   bpg_image_queue_tail = (bpg_image_queue_tail+1)%BPG_IMAGE_QUEUE_SIZE;
  ...
  if (pb!=pb2) {
    ...
    if (tr==0) { packet_measurechunkhead=packet_measurechunktail;}
    {
      while (packet_measurechunkhead != packet_measurechunktail) {
        ...
        if (mc->beat_buffer_current_frame > tr) {
	  logit("GONE %ld %ld\n",mc->beat_buffer_current_frame,tr);
          packet_measurechunkhead = (packet_measurechunkhead+1) % MEASURE_CHUNK_COUNT;
	  redo_song_frame = 1;
	  }
        else if (mc->beat_buffer_current_frame+ mc->beat_buffer_length + 7000 < tr) { // long time ago
	  logit("GONE2 %ld %ld\n",mc->beat_buffer_current_frame,tr);
          packet_measurechunkhead = (packet_measurechunkhead+1) % MEASURE_CHUNK_COUNT;
	  redo_song_frame = 1;
	  }
...
if (packet_summary.command_show_ip_address_stuff.show_mode) {
  if (time_difference_in_milliseconds(packet_summary.current_sample_time,packet_summary.command_show_ip_address_stuff.stop_show_mode_time)>=0) {
    packet_summary.command_show_ip_address_stuff.show_mode=0;
    return;
    } /* if we are done showing the positional color */
//  logit("diff %d",time_difference_in_milliseconds(packet_summary.current_sample_time,packet_summary.command_show_ip_address_stuff.stop_show_mode_time));
...
#ifdef COLLECT_BPG_IMAGES
memset(bpg_images,0,sizeof(void *));
bpg_image_queue_head=0;
bpg_image_queue_tail=0;
#endif

packet_summary.state=PACKET_STATE_NOTHING;
packet_summary.current_frame=-1;
...
packet_summary.soundringcurrent=-1;
...
packet_summary.beat_interval_in_milliseconds=500000;
int i;
...
long ms;
ms = (long)(a.tv_sec-b.tv_sec)*1000000 + (long)(a.tv_usec - b.tv_usec);
return ms;
}

struct timeval add_milliseconds_to_time (struct timeval a,long milliseconds) {
if (milliseconds>=0) {
  a.tv_usec = a.tv_usec + milliseconds;
  ...
  a.tv_sec += a.tv_usec/1000000;
  a.tv_usec = a.tv_usec % 1000000;
  if (a.tv_usec<0)  a.tv_usec += 1000000;
  }
return a;
}
...
if (analyzing) {
  if ((head==tail)&&(first==tail)) {
    ...
    current=head-1;
    if (current<0) current += SOUNDRING_COUNT;
    }
  else {
    current=head;
    }
...
if (current != -1)
  packet_summary.current_frame = soundstat[current]->frame;
    else
        packet_summary.current_frame=-1;
    
if (packet_summary.current_frame<=0) {
  packet_summary.current_frame = soundstat[soundringhead]->frame;
  }
...
//  logit("  f is soundstat[%d]->frame = %d",first,f);
  if ((packet_summary.start_frame==-1)||(packet_summary.start_frame-f>PACKET_SUMMARY_SIZE)||
    ...
    (packet_summary.start_frame-packet_summary.current_frame>PACKET_SUMMARY_SIZE)||
    (packet_summary.current_frame-packet_summary.start_frame>PACKET_SUMMARY_SIZE)    ) {
...
packet_summary.soundringcurrent = current;
...
{ /* find no_gap_frame */
  packet_summary.no_gap_frame = packet_summary.current_frame;
...
else if ((packet_summary.current_frame != -1)&&(packet_summary.state!=PACKET_STATE_WARMING)) { 
  int quality_count=0;
  float tally=0.;
  int startframe = packet_summary.current_frame-80+1;
  if (startframe< packet_summary.no_gap_frame)  startframe=packet_summary.no_gap_frame;
  int f;
  for (f=startframe;f<=packet_summary.current_frame;f++) {
    int index = (packet_summary.start_index + f - packet_summary.start_frame) % PACKET_SUMMARY_SIZE;
    if (index<0) index += PACKET_SUMMARY_SIZE;
    struct packet *packet = packet_summary.packets+index;  
...
if (packet_summary.current_frame != -1) { 
    int index = (packet_summary.start_index + packet_summary.current_frame - packet_summary.start_frame) % PACKET_SUMMARY_SIZE;
    if (index<0) index += PACKET_SUMMARY_SIZE;
    struct packet *packet = packet_summary.packets+index;  
    if (packet->has_statistics) {
        int good=1;
	int bad=0;
	int difference=time_difference_in_milliseconds(packet_summary.current_sample_time,packet_summary.tempo_points[0]);
...
//	logit("diff %d bad %d good %d",difference,bad,good);
	...
	if (bad) {
	  ...
	  packet_summary.tempo_points[0] = packet_summary.current_sample_time;
	  packet_summary.where_in_tempo=0.;
	  packet_summary.beat_interval_in_milliseconds= difference;
          }
	else {
	  if (packet_summary.tempo_points[1].tv_sec != 0) {
  	    int difference2=time_difference_in_milliseconds(packet_summary.tempo_points[0],packet_summary.tempo_points[1]);
	    packet_summary.where_in_tempo = (float)((double)(difference)/(double)(difference2));
	    packet_summary.beat_interval_in_milliseconds= difference2;
	    }
	  else {
	    packet_summary.where_in_tempo = (float)((double)(difference)/(double)(500000)); /* assume a tempo of 0.5 */
            packet_summary.beat_interval_in_milliseconds= 500000;
	    }
	  }	
...
int i;
for (i=0;i<65536;i++) {
  struct bpg_image *image = bpg_images[i];
  if (image) {
    if (image->chunk_map) free(image->chunk_map);
    if (image->image) free(image->image);
    free(image);
...
//logit("		set %ld to %d ps %d\n",song_frame,stream_frame,packet_summary.current_frame);
...
while  (packet_measurechunktail!=m) {
  ...
  if (the_frame > (mc->beat_buffer_current_frame+mc->beat_buffer_length)) {
    m = (m+1) % MEASURE_CHUNK_COUNT;
    }
...
#endif
int cframe = packet_summary.current_frame-sound_delay_frames;
...
extern void increase_demo_time(int milliseconds) {
if (make_demo_mode) {
  demo_time_of_day = add_milliseconds_to_time((
    (struct timeval)demo_time_of_day),milliseconds);
  }
}

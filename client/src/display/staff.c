...
if (!ran_setup_graphics_once_before)
    ...
    checkGlError("glGetUniformLoaction");
    matrixAttrLoc = glGetAttribLocation(gProgram, "myMatrix");
...
static void draw_a_line(int one_color_program,int mColorHandle,int poshandle,float xfrom,float yfrom,float xto,float yto,float red,float green,float blue) {
int n=0;
float vertices[4];
...
// draw_a_line (p,clrhandle,-0.2,-0.2,0.43,0.43,0.,0.5,1.);
 /* red square is actually 2 red triangles
the flag is 1.8 instead of 400, so 1.8/5 is .. 0.36
*/
//draw_a_triangle(p,clrhandle,-0.9, -0.9,   0.9,-0.9,  0.9,-0.9+0.36, 
//                 1., 0.,0.); 

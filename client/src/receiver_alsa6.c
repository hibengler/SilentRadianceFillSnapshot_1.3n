...
//#define WORLD_receiver_alsa6_default "default"
    rc = snd_pcm_open(&handle, WORLD_receiver_alsa6_default,
                    SND_PCM_STREAM_PLAYBACK,SND_PCM_NONBLOCK);
    }
  else {
//#define WORLD_receiver_alsa6_default "default"
    rc = snd_pcm_open(&handle, WORLD_receiver_alsa6_default,0,
...
if ((sock = socket(PF_INET6, SOCK_DGRAM|SOCK_NONBLOCK, IPPROTO_UDP)) < 0)
...
//broadcastAddr.sin6_addr.s_addr = htonl(INADDR_ANY); /* Any incoming interface */
...
write_file_spec =fopen(WORLD_record_dot_srd,WORLD_receiver_alsa6_w);
/* init netrings */
{
  int i;
...
FILE *read_file_spec =fopen(argv[1],WORLD_receiver_alsa6_r);
if (!read_file_spec) {

...
#include "aubio_config.h"

#if HAVE_JACK
#include "aubio_utils.h" // for aubio_process_func_t
...
new_aubio_jack (uint_t hop_size, uint_t ichan, uint_t ochan,
    uint_t imidichan, uint_t omidichan,char *client_name)
{
  ...
  jack_setup->imat = new_fmat(2,hop_size);
...
//    fvec_set_sample(dev->ibuf, input[0][j], dev->pos);
    fmat_set_sample(dev->imat, input[0][j],0,dev->pos);
    fmat_set_sample(dev->imat, input[1][j],1,dev->pos);
    /*time for fft*/
    if (dev->pos == (int)(dev->hop_size) - 1) {
      /* block loop */
      dev->callback(dev->imat, dev->obuf);
      /* end of block loop */

...
echo ifconfig
sudo ifconfig wlan0 10.71.0.1/16 up
echo sleeping
sleep 2
ifconfig wlan0
echo start hdcp
service isc-dhcp-server stop
cp dhcpd3.conf /etc/dhcp/dhcpd.conf
...
iptables -t nat -A POSTROUTING -s 10.71.0.0/16 ! -d 10.71.0.0/16  -j MASQUERADE 
echo hostapd
hostapd /etc/hostapd/hostapd3.conf

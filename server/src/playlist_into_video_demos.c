...
//  ww=7;
  ...
  fprintf(stdout,"srplayer_to_sirad %s >a.siradx\n",filename);
  fprintf(stdout,"rm 2>/dev/null a.raw* a.rawrgb\n");
  char quoted_title[2000];
  handle_quotes(quoted_title,title);
  fprintf(stdout,"echo \"\n\n\n\" Part 1 doing %s\n",quoted_title);
    
  fprintf(stdout,"silentradiance_make_demo a.siradx 40 15 %s\n",quoted_title);
  ...
  fprintf(stdout,"echo \"\n\n\n\" Part 2 doing %s\n",quoted_title);
  fprintf(stdout,"%s </dev/null  -f s16le -ar 48000 -ac 2 -i a.rawsound  -c:a aac -strict experimental "
   " -bsf:a aac_adtstoasc -b:a %dk  a.aac\n",FFMPEG_COMMAND,TARGET_AUDIO_BITRATE);
/* note - one pass is here:
ffmpeg -v debug -framerate 30 -f rawvideo -pix_fmt rgb24 -s 480x480 -i a.rawrgb \
  -c:v libx264   -pix_fmt yuv420p -profile:v main -level 3.1 -movflags +faststart -crf 28 a_ns.mp4
  # tried 23,26  ,28
  */
  fprintf(stdout,"%s </dev/null -y -framerate 30 -f rawvideo -pix_fmt rgb24 -s 480x480 -i a.rawrgb "
    "-c:v libx264  -pass 1  -pix_fmt yuv420p -preset veryslow -b:v %dk -an -profile:v main -level 3.1 "
    "  -movflags +faststart -r 30 -f mp4 /dev/null\n",FFMPEG_COMMAND,TARGET_VIDEO_BITRATE);
  fprintf(stdout,"%s </dev/null  -framerate 30 -f rawvideo -pix_fmt rgb24 -s 480x480 -i a.rawrgb "
    " -c:v libx264 -pass 2  -pix_fmt yuv420p -preset veryslow  -b:v %dk -an -profile:v main -level 3.1 "
    " -movflags +faststart a_ns.mp4\n",FFMPEG_COMMAND,TARGET_VIDEO_BITRATE);
  fprintf(stdout,"%s </dev/null -i a_ns.mp4 -i a.aac -acodec copy  -bsf:a aac_adtstoasc -vcodec copy a.mp4\n",
      FFMPEG_COMMAND);
  fprintf(stdout,"echo \"\n\n\n\" Part 3 copying %s\n",quoted_title);
...
/*
  $Log: playlist_into_video_demos.c,v $
  Revision 1.5  2019/06/01 02:25:43  hib
  wip

  Revision 1.4  2019/05/31 00:56:35  hib
  compiled and built on bump server - updated some 18.04 stuff
  Stopped building aubio from sorurce for 18.04

  Revision 1.3  2019/05/20 02:59:27  hib
  well wip

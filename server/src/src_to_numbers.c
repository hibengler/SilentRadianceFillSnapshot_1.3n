...
int convert_to_wav(unsigned char *packet,int sampleSize) {
...
{/* Opus frame - including recovery */
  ...
  for (backup_frame_number=opus_frames-1;backup_frame_number>=0;backup_frame_number--) {
    ...
    for (i=0;i<240;i++) {
      ...
      short s2;
      s= (int)(pcm[i+i]*32768.);     
      s2= (int)(pcm[i+i+1]*32768.);
      fprintf(stdout,"n %d	f %d	%d	%d	%d\n",newframe,frame,i,s,s2);
      }
...
if (argc<2) {
  fprintf(stderr,"usage: src_to_wav source_file.src >dest.txt\n");
  exit(2);
  }
...
while (fread(bytes_array,2,1,read_file_spec)) {
  ...
  convert_to_wav(buffer,packet_size);
  }

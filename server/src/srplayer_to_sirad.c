...
#include <errno.h>
  ...
  long file_frame=0;
  int number_chunks=12000; // 60 seconds
...
{ /* make ibuf form stereo matrix */
  ...
  for (i=0;i<imat->length;i++) { 
    int k=0;
      { // was a slow_down_speed for loop 
...
//        playOneSoundBuffer();	  
        ...
        else if ((number_chunks)&&(current_chunk_number>start_chunk_number
	   +number_chunks)) {
	   fprintf(stderr,"n %d c %d s %d\n",number_chunks,current_chunk_number,start_chunk_number);
	  exit(0);
	  }
	else {
//	  play_one((char *)out1);
          int rc=music_broadcast_buffer_write((char *)out1);
	  if (rc) exit(rc);
	  }
...
char *argvf2[] = {"srplayer_to_sirad","STREAM_SR00","OK","","","","","","","","","","","","","","","","","","","","",""};
...
//  init_receiver_sound(0);
  music_broadcast_from_buffer(3,argvf2);
...
//  for (i=0;i<argc+2;i++) {fprintf(stderr," %s\n",argvf[i]  );}
  ...
  c_examples_common_init(argc+2,argvf,"hi",0);
  setup_shared_memory();
  if (sr_c) {
//    fprintf(stderr,"linked to shared memory broadcaster\n");
  ...
  c_examples_common_process((aubio_process_func_t)process_block_c, process_print_c);
...
c_examples_common_del();
      
}

		  
/* 
  $Log: srplayer_to_sirad.c,v $
  Revision 1.2  2019/03/14 12:47:55  hib
  wip - working on makeing the store look good

  Revision 1.1  2018/12/22 02:53:28  hib
